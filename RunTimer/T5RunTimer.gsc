// T5 ZM - Run Timer (ported from the T6 counter's timer panel)
//
...
//
// Sits directly below the enemy counter and follows it when the counter grows
// for the dog row, or takes the top slot on a server that has no counter.
// Standalone: it only ever reads the counter's state.
...
_rt_build_hud()
{
    ...
    // The counter builds on the same notify; give it a moment so we know
    // whether this server has one to sit below.
    wait 1;

    player.rt_enabled = level._rt_enabled_default;
    player.rt_split_showing = false;

    // Same visual language and width as the counter, stacked below it. A
    // server without the counter has nothing above us, so we take its place.
    px = 20;
    py = player _rt_target_py();
    w = 80;
    ...
    player thread _rt_follow_counter();
}
...
// Where the panel belongs: under the counter, 13 lower again while the
// counter is grown for its dog row, or at the top on a server with no
// counter at all. Reading the counter's own state keeps this script
// standalone - the counter never has to know about us.
_rt_target_py()
{
...
_rt_move( py )
{
    ...
    self.rt_accent.y = py;
    self.rt_sep.y = py + 13;
    self.rt_title.y = py + 2;
    self.rt_total_label.y = py + 17;
    self.rt_total_time.y = py + 17;

    self.rt_split_bg.y = py + 30;
    self.rt_split_accent.y = py + 30;
    self.rt_split_label.y = py + 31;
    self.rt_split_time.y = py + 31;
}
