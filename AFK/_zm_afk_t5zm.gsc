...
player_is_downed()
{
    fn = getFunction( "maps/_laststand", "player_is_in_laststand" );
    if ( isDefined( fn ) )
        return self [[ fn ]]();
    // Fallback: check sessionstate (less precise but always available)
    return ( self.sessionstate == "spectator" && isAlive( self ) );
}
...
// ============================================================
// Player Connection
// T5ZM uses "connecting" (not "connected") for player join events
// ============================================================
...
on_player_connect()
{
    ...
    for ( ;; )
    {
        // T5ZM: use "connecting" here, not "connected"
        level waittill( "connecting", player );

        println( "[AFK] player connecting: " + player.playername );
...
capture_spawn_position()
{
    ...
    if ( !isDefined( level.afk_system.verified_spawn ) )
    {
        ...
        println( "[AFK] captured verified spawn from " + self.playername
                 + " at (" + int( self.origin[0] ) + ","
                 + int( self.origin[1] ) + ","
                 + int( self.origin[2] ) + ")" );
    }
}
...
get_afk_spawn_point()
{
    if ( isDefined( level.afk_system.verified_spawn ) )
    {
        ...
        point.angles  = ( 0, 0, 0 );
        println( "[AFK] using verified spawn at ("
                 + int( point.origin[0] ) + ","
                 + int( point.origin[1] ) + ","
                 + int( point.origin[2] ) + ")" );
        return point;
...
try_toggle_afk()
{
    ...
    // T5ZM: laststand check via getFunction wrapper
    if ( self player_is_downed() )
    {
...
afk_activation_delay()
{
    ...
    for ( i = delay; i > 0; i-- )
    {
        if ( !isAlive( self ) || self player_is_downed() )
        {
...
afk_position_lock()
{
    ...
    for ( ;; )
    {
        ...
        if ( distanceSquared( self.origin, locked_origin ) > 4 )
        {
            self setOrigin( locked_origin );
            self setPlayerAngles( locked_angles );
        }
    }
}
...
freeze_round()
{
    ...
    println( "[AFK] freeze: alive=" + alive_at_freeze
             + " queue=" + level.zombie_total
             + " budget=" + level.afk_system.saved_round_zombies );
...
unfreeze_round()
{
    ...
    println( "[AFK] unfreeze: alive=" + alive_now
             + " budget=" + level.afk_system.saved_round_zombies
             + " new_queue=" + new_queue );
...
debug_hud_player()
{
    ...
    for ( ;; )
    {
        ...
        self.afk_dbg_line1 setText( "^6[AFK] ^7" + state
            + " ^7al:" + alive_str
            + " dn:" + downed_str
            + " r:" + round_num
            + " cd:" + cd_str );
        ...
        self.afk_dbg_line2 setText( "^6[AFK] ^7frz:" + frozen_str
            + " za:" + zalive
            + " zq:" + ztotal
            + " p:" + active_c + "/^1" + afk_c );
